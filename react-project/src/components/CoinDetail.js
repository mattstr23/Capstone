...
export default function CoinDetail() {
  ...
  const fetchInfo = async () => {
    const getCoin = await fetch(
      `https://api.coingecko.com/api/v3/coins/${location.id}?tickers=true`
    );
      ...
      <div className="cryptoInfo">
        <div className="mainDetailCont">
          ...
          <div className="priceDifferenceCont">
            <p className="differenceHigh">
              ${info?.market_data?.high_24h?.usd?.toLocaleString()}
            </p>
            <p className="differenceLow">
              ${info?.market_data?.low_24h?.usd?.toLocaleString()}
            </p>
          </div>
          <Link to="/markets">
